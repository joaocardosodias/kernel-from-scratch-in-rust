...
#[derive(Clone, Copy)]
pub struct Entry {
    pub offset_low: u16, pub code_selector: u16, pub ist_and_flags: u16,
    pub offset_mid: u16, pub offset_high: u32, pub reserved: u32,
}
#[repr(C)]
pub struct IDT { pub entries: [Entry; 256], }
#[repr(C)]
pub struct InterruptStackFrame {
    pub instruction_pointer: u64, pub code_segment: u64, pub cpu_flags: u64,
    pub stack_pointer: u64, pub stack_segment: u64,
}

#[no_mangle]
pub extern "x86-interrupt" fn divide_by_zero_handler(_stack_frame: &mut InterruptStackFrame) {
    println!("Divided by zero"); loop {}
}
pub extern "x86-interrupt" fn invalid_opcode(_stack_frame: &mut InterruptStackFrame) {
    println!("Invalid Opcode"); loop {}
}

pub extern "x86-interrupt" fn general_protection_fault(_stack_frame: &mut InterruptStackFrame, _error_code: u64) {
    let vga = 0xB8000 as *mut u8;
    let msg = b"GPF!";
    for (i, &b) in msg.iter().enumerate() {
        unsafe { vga.add(i * 2).write(b); vga.add(i * 2 + 1).write(0x04); }
    }
    loop {}
}
...
pub extern "x86-interrupt" fn page_fault(_stack_frame: &mut InterruptStackFrame, _error_code: u64) {
    let vga = 0xB8000 as *mut u8;
    unsafe { vga.add(0).write(b'P'); vga.add(1).write(0x0F); }

    let fault_addr: u64;
    unsafe { core::arch::asm!("mov {}, cr2", out(reg) fault_addr); }
    unsafe { vga.add(2).write(b'F'); vga.add(3).write(0x0F); }

    if fault_addr >= 0x200000 && fault_addr < 0x300000 {
        unsafe { vga.add(4).write(b'H'); vga.add(5).write(0x0F); }
        
        crate::memory::map_page(fault_addr);
        
        unsafe { vga.add(6).write(b'M'); vga.add(7).write(0x0F); }
    } else {
        loop {}
    }
}
...
pub extern "x86-interrupt" fn time_handler(_stack_frame: &mut InterruptStackFrame) {
    unsafe { core::arch::asm!("out 0x20, al", in("al") 0x20u8 as i8); }
    println!("Time handler")
}
...
impl IDT {
    pub fn set_entry(&mut self, vector: u8, handler: u64) {
        ...
        self.entries[vector as usize] = Entry {
            offset_low, code_selector, ist_and_flags,
            offset_mid, offset_high, reserved: 0,
        }
    }
}
#[repr(C, packed)]
pub struct Pointer { limit: u16, base: u64 }
impl IDT {
    pub fn load(&self) {
        unsafe {
            let ptr = Pointer { limit: (256 * 16 - 1) as u16, base: self as *const _ as u64 };
